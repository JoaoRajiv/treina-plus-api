...
2. Se o usuário **não tem dados cadastrados** (retornou null):
    - Pergunte nome, peso (kg), altura (cm), idade e % de gordura corporal (inteiro de 0 a 100).
		...
		handler: async (request, reply) => {
			try {
				...
				const result = streamText({
					...
					onError: ({ error }) => {
						app.log.error({ err: error }, "AI stream failed");
					},
					tools: {
						...
						updateUserTrainData: tool({
							...
							inputSchema: z.object({
								...
								bodyFatPercentage: z
									...
									.min(0)
									.max(100)
									.describe("Percentual de gordura corporal inteiro (0 a 100)"),
							}),
				...
				app.log.error({ err: error }, "AI request failed");
